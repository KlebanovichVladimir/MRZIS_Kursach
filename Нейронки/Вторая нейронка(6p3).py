...
# Параметры системы
t0, x0, y0, z0, step_size, n_steps = 0.0, 0.5, 0.5, 0.5, 0.1, int(100 / 0.1)
...
# Визуализация результатов на тестовых и тренировочных данных
for data_type, true_data, pred_data, data_label in [
    ...
    for i, var in enumerate(["X", "Y", "Z"]):
        ...
        plt.plot(range(len(pred_data)), pred_data[:, i], label='График предсказания', linestyle='--')
    ...
    # Построение графиков зависимостей Y(X), Z(X), Z(Y)
    plt.figure(figsize=(12, 6))
    plt.plot(true_data[:, 0], true_data[:, 1], label='Y(X) эталон', alpha=0.7)
    plt.plot(pred_data[:, 0], pred_data[:, 1], label='Y(X) предсказание', linestyle='--', alpha=0.7)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title(f'График Y(X) на {data_type} данных')
    plt.legend()
    plt.show()

    plt.figure(figsize=(12, 6))
    plt.plot(true_data[:, 0], true_data[:, 2], label='Z(X) эталон', alpha=0.7)
    plt.plot(pred_data[:, 0], pred_data[:, 2], label='Z(X) предсказание', linestyle='--', alpha=0.7)
    plt.xlabel('X')
    plt.ylabel('Z')
    plt.title(f'График Z(X) на {data_type} данных')
    plt.legend()
    plt.show()

    plt.figure(figsize=(12, 6))
    plt.plot(true_data[:, 1], true_data[:, 2], label='Z(Y) эталон', alpha=0.7)
    plt.plot(pred_data[:, 1], pred_data[:, 2], label='Z(Y) предсказание', linestyle='--', alpha=0.7)
    plt.xlabel('Y')
    plt.ylabel('Z')
    plt.title(f'График Z(Y) на {data_type} данных')
    plt.legend()
    plt.show()